...
@Repository class EquipeRepositorioJdbc implements EquipeRepositorio{private final JdbcTemplate jdbc;EquipeRepositorioJdbc(JdbcTemplate j){jdbc=j;}private static final String BASE="SELECT id,nome,email,senha_hash,papel::text,status_presenca::text,ativo FROM usuario";
 ...
 @Override public Optional<Usuario> porId(UUID id){return jdbc.query(BASE+" WHERE id=?",EquipeRepositorioJdbc::mapear,id).stream().findFirst();}private static Usuario mapear(ResultSet r,int i)throws SQLException{return new Usuario(r.getObject("id",UUID.class),r.getString("nome"),r.getString("email"),r.getString("senha_hash"),PapelUsuario.valueOf(r.getString("papel")),StatusPresenca.valueOf(r.getString("status_presenca")),r.getBoolean("ativo"));}}
