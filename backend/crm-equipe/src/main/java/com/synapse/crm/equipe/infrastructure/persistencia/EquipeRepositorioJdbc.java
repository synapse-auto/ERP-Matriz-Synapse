...
@Repository class EquipeRepositorioJdbc implements EquipeRepositorio{private final JdbcTemplate jdbc;EquipeRepositorioJdbc(JdbcTemplate j){jdbc=j;}private static final String BASE="SELECT u.id,u.nome,u.email,u.senha_hash,u.papel::text,u.status_presenca::text,u.ativo,u.telefone,u.cargo,u.senha_alterada_em,COALESCE(d.disponivel_para_ia,FALSE) AS disponivel_para_ia FROM usuario u LEFT JOIN disponibilidade_atendente_ia d ON d.atendente_id=u.id";
 ...
 @Override public Usuario criar(String n,String e,String h,PapelGerenciavel p){UUID id=UUID.randomUUID();try{jdbc.update("INSERT INTO usuario(id,nome,email,senha_hash,papel) VALUES(?,?,?,?,CAST(? AS papel_usuario))",id,n,e,h,p.name());if(p==PapelGerenciavel.ATENDENTE)jdbc.update("INSERT INTO disponibilidade_atendente_ia(atendente_id,disponivel_para_ia) VALUES(?,TRUE)",id);}catch(DuplicateKeyException x){throw new EmailDeUsuarioEmUsoException();}return porId(id).orElseThrow();}
 @Override public Optional<Usuario> atualizar(UUID id,String n,String e,PapelGerenciavel p){try{int x=jdbc.update("UPDATE usuario SET nome=?,email=?,papel=CAST(? AS papel_usuario) WHERE id=? AND papel IN ('ATENDENTE','SUBGESTOR')",n,e,p.name(),id);if(x==1&&p==PapelGerenciavel.ATENDENTE)jdbc.update("INSERT INTO disponibilidade_atendente_ia(atendente_id,disponivel_para_ia) VALUES(?,TRUE) ON CONFLICT (atendente_id) DO NOTHING",id);return x==0?Optional.empty():porId(id);}catch(DuplicateKeyException x){throw new EmailDeUsuarioEmUsoException();}}
