...
class ParticipacaoAtendimentoRepositorioJdbc implements ParticipacaoAtendimentoRepositorio {
    ...
    public Optional<PedidoEntradaAtendimento> pedido(UUID id) { return jdbc.query("SELECT p.*, u.nome FROM pedido_entrada_atendimento p JOIN usuario u ON u.id=p.solicitante_id WHERE p.id=?", (r,i)->mapear(r), id).stream().findFirst(); }
    public Optional<PedidoEntradaAtendimento> pedidoDoSolicitante(UUID atendimentoId, UUID solicitanteId, Instant limite) {
        return jdbc.query("SELECT p.*, u.nome FROM pedido_entrada_atendimento p JOIN usuario u ON u.id=p.solicitante_id WHERE p.atendimento_id=? AND p.solicitante_id=? ORDER BY p.solicitado_em DESC LIMIT 1", (r,i)-> {
            ...
            return new PedidoEntradaAtendimento(r.getObject("id",UUID.class),r.getObject("atendimento_id",UUID.class),r.getObject("solicitante_id",UUID.class),r.getString("nome"),status,TipoPedidoEntrada.valueOf(r.getString("tipo")),r.getTimestamp("solicitado_em").toInstant());
        }, atendimentoId, solicitanteId).stream().findFirst();
    }
    public List<PedidoEntradaAtendimento> pendentes(UUID a, Instant limite) { return jdbc.query("SELECT p.*, u.nome FROM pedido_entrada_atendimento p JOIN usuario u ON u.id=p.solicitante_id WHERE p.atendimento_id=? AND p.status='PENDENTE' AND p.tipo='SOLICITACAO' AND p.solicitado_em > ?", (r,i)->new PedidoEntradaAtendimento(r.getObject("id",UUID.class),r.getObject("atendimento_id",UUID.class),r.getObject("solicitante_id",UUID.class),r.getString("nome"),StatusPedidoEntrada.PENDENTE,TipoPedidoEntrada.valueOf(r.getString("tipo")),r.getTimestamp("solicitado_em").toInstant()), a, Timestamp.from(limite)); }
    public ConviteResultado convidar(UUID atendimentoId, UUID convidadoId, Instant agora) {
        TransacaoObrigatoria.exigir("convidarParaAtendimento");
        int inserido = jdbc.update("""
                INSERT INTO pedido_entrada_atendimento (atendimento_id, solicitante_id, tipo, solicitado_em)
                SELECT ?, ?, 'CONVITE', ?
                 WHERE EXISTS (SELECT 1 FROM atendimento WHERE id=? AND status <> 'FINALIZADO')
                   AND NOT EXISTS (SELECT 1 FROM atendimento_participante
                                    WHERE atendimento_id=? AND usuario_id=? AND saiu_em IS NULL)
                ON CONFLICT DO NOTHING
                """, atendimentoId, convidadoId, Timestamp.from(agora), atendimentoId, atendimentoId, convidadoId);
        UUID id = jdbc.query("SELECT id FROM pedido_entrada_atendimento WHERE atendimento_id=? AND solicitante_id=? AND status='PENDENTE'", (r,i)->r.getObject(1, UUID.class), atendimentoId, convidadoId).stream().findFirst().orElse(null);
        return id == null ? new ConviteResultado(null, false) : new ConviteResultado(id, inserido > 0);
    }
    ...
    public void aprovar(UUID p, UUID dono, Instant agora) { int n=jdbc.update("UPDATE pedido_entrada_atendimento p SET status='APROVADO',respondido_em=?,respondido_por_id=? WHERE p.id=? AND p.status='PENDENTE' AND EXISTS(SELECT 1 FROM atendimento a WHERE a.id=p.atendimento_id AND a.status<>'FINALIZADO' AND ((p.tipo='SOLICITACAO' AND a.atendente_id=?) OR (p.tipo='CONVITE' AND p.solicitante_id=?)))",Timestamp.from(agora),dono,p,dono,dono); if(n==0) throw new IllegalStateException("pedido expirado, recusado ou atendimento indisponivel"); jdbc.update("INSERT INTO atendimento_participante(atendimento_id,usuario_id) SELECT atendimento_id,solicitante_id FROM pedido_entrada_atendimento WHERE id=? ON CONFLICT DO NOTHING",p); }
    public void recusar(UUID p, UUID dono, Instant agora) { int n=jdbc.update("UPDATE pedido_entrada_atendimento p SET status='RECUSADO',respondido_em=?,respondido_por_id=? WHERE p.id=? AND p.status='PENDENTE' AND EXISTS(SELECT 1 FROM atendimento a WHERE a.id=p.atendimento_id AND a.status<>'FINALIZADO' AND ((p.tipo='SOLICITACAO' AND a.atendente_id=?) OR (p.tipo='CONVITE' AND p.solicitante_id=?)))",Timestamp.from(agora),dono,p,dono,dono); if(n==0) throw new IllegalStateException("pedido indisponivel"); }
    ...
    private PedidoEntradaAtendimento mapear(java.sql.ResultSet r) throws java.sql.SQLException {
        return new PedidoEntradaAtendimento(r.getObject("id",UUID.class),r.getObject("atendimento_id",UUID.class),r.getObject("solicitante_id",UUID.class),r.getString("nome"),StatusPedidoEntrada.valueOf(r.getString("status")),TipoPedidoEntrada.valueOf(r.getString("tipo")),r.getTimestamp("solicitado_em").toInstant());
    }
}
