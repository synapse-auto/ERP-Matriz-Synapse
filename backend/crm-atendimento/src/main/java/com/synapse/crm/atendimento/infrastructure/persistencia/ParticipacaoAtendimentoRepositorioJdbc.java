...
class ParticipacaoAtendimentoRepositorioJdbc implements ParticipacaoAtendimentoRepositorio {
    ...
    public List<ParticipanteAtendimento> ativos(UUID a) { return jdbc.query("SELECT p.usuario_id,u.nome,p.entrou_em FROM atendimento_participante p JOIN usuario u ON u.id=p.usuario_id WHERE p.atendimento_id=? AND p.saiu_em IS NULL ORDER BY p.entrou_em",(r,i)->new ParticipanteAtendimento(r.getObject(1,UUID.class),r.getString(2),r.getTimestamp(3).toInstant()),a); }
}
