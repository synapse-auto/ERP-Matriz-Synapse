...
Antes de autorizar o deploy, rode a simulacao somente-leitura e guarde toda a saida:

```bash
docker exec -i "$container" psql -U "$SYNAPSE_DB_USER" -d "$SYNAPSE_DB_NAME" \
  -v ddi="${TELEFONE_DDI_PADRAO:-55}" \
  < docker/provisionamento/simular-limpeza-prefixo-discagem.sql \
  | tee /caminho/seguro/relatorios/e180-prefixo-$(date +%Y%m%d-%H%M%S).txt
```
