...
| GET | `/api/v1/atendimentos/busca?leadId=...` ou `?telefone=...` | Busca pontualmente o cartão representativo mais recente de um lead visível; exige exatamente um parâmetro e normaliza telefone pelo contrato canônico. 404 igual para inexistente e fora do alcance (lead de colega). Usado pelo "Abrir conversa" do card de contato compartilhado (E211) | Atendente | `PainelDeAtendimentosController` · `PainelDeAtendimentosControllerIT` |
