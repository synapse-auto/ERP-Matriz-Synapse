...
A grade de KPIs da Visão Geral **não mostra mais o card "Vendas fechadas"** — o lugar ficou com **Avaliação** (`avg(avaliacao.nota)` na escala 1–5). Vendas continuam no payload e na taxa de conversão. O ranking da Visão Geral passou a ser por avaliação. A coleta é `POST /api/v1/atendimentos/{id}/avaliacao` e `POST /internal/v1/atendimentos/{id}/avaliacao` (Automação). Desde a E124, a finalização não dispara solicitação automaticamente; o n8n conduz o início do fluxo. Sem linha em `avaliacao`, o card continua mostrando vazio de verdade.
...
O composer mantém o botão de anexo (`RF-CRM-09`, `RF-CRM-68`): o clipe abre um popover para cima com uma grade apenas de ícones, cada um com tooltip e nome acessível. A grade oferece **Arquivos** (upload do computador, vários de uma vez), **Templates** (templates WhatsApp aprovados, o mesmo fluxo da janela de 24h) e **Mensagens rápidas** (o fluxo existente de escolha e preenchimento de texto). Também dá para arrastar arquivos do explorador para a coluna do chat (histórico + composer). Cada arquivo segue o `POST` de mídia existente, em sequência. O que sai é o **repositório compartilhado** de arquivos frequentes (`RF-CRM-13/55/56`).
