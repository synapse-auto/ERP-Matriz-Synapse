...
- Os comandos operacionais da Automação usam somente `/internal/v1`, autenticados por `X-Synapse-Token` e `ROLE_SERVICO`: responder como IA, transferir para um atendente ativo validado, devolver para IA e transferir para o próximo humano elegível. Responder grava `Remetente.IA` e a outbox na mesma transação, sem chamar o provedor no request; a Automação não altera o dono pela RN-CRM-06 e não informa um destino arbitrário na rota de próximo humano.
