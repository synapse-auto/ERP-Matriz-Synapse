...
- Consultas e comandos operacionais da Automação usam somente `/internal/v1`, autenticados por `X-Synapse-Token` e `ROLE_SERVICO`. Além de responder/transferir, esse contrato lista atendimentos não encerrados por última atividade, cria lembrete idempotente para o responsável atual, sobrescreve o resumo da IA e aplica somente tags do catálogo existente. A Automação não consome a visão de tela de `/api/v1/atendimentos`, não recebe histórico na listagem interna e nunca cria tag implicitamente.
- Responder grava `Remetente.IA` e a outbox na mesma transação, sem chamar o provedor no request; a Automação não altera o dono pela RN-CRM-06 e não informa um destino arbitrário na rota de próximo humano.
