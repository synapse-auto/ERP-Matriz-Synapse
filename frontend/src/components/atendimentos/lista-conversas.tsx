...
import { MoreHorizontal, Search, SlidersHorizontal, UserPlus, Users, UsersRound } from "lucide-react";
...
export function ListaConversas({
  ...
  const [novoGrupoInternoAberto, setNovoGrupoInternoAberto] = useState(false);
    ...
    <div className={cn("flex h-full min-h-0 flex-col overflow-hidden border-r border-border bg-background", className)}>
            ...
            {chatInternoHabilitado && (
              <>
                ...
                <Button
                  ...
                  size="icon-sm"
                  className="min-h-10 min-w-10"
                  aria-label={catalogo.chatInterno.novoGrupo}
                  title={catalogo.chatInterno.novoGrupo}
                  aria-expanded={novoGrupoInternoAberto}
                  onClick={() => {
                    setNovoGrupoInternoAberto(true);
                    onRecarregarContatos?.();
                  }}
                >
                  <Users className="size-(--tamanho-icone-interface)" aria-hidden />
                </Button>
              </>
            )}
      ...
      {chatInternoHabilitado && (
        <>
          ...
          />
          <DialogoCriarGrupo
            aberto={novoGrupoInternoAberto}
            onFechar={() => setNovoGrupoInternoAberto(false)}
            contatos={contatosInternos}
            onCriar={(nome, participantes) => onCriarGrupoInterno?.(nome, participantes) ?? Promise.resolve()}
            textos={catalogo.chatInterno}
          />
        </>
      )}
