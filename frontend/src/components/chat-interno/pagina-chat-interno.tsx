  ...
  return <div className="flex h-full min-h-0 flex-col gap-5 p-6">
    ...
    <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[300px_1fr]">
      <Card className="min-h-0"><CardHeader className="flex flex-row items-center justify-between"><CardTitle>{textos.conversas}</CardTitle><Button size="icon" variant="outline" aria-label={textos.novaConversa} onClick={() => contatos.refetch()}><Plus /></Button></CardHeader><CardContent className="space-y-2 overflow-y-auto">
        ...
        {conversas.data?.map((c) => <button key={c.id} type="button" aria-current={c.id === conversaId ? "true" : undefined} className="flex w-full items-center gap-3 rounded-xl border border-transparent p-3 text-left hover:bg-muted aria-[current=true]:border-primary/20 aria-[current=true]:bg-primary/10" onClick={() => setConversaId(c.id)}><AvatarIniciais id={c.id} nome={c.participantes || textos.titulo} className="flex size-10 shrink-0 items-center justify-center rounded-xl text-xs font-bold text-white" /><span className="min-w-0 flex-1"><span className="block truncate font-medium">{c.participantes || textos.titulo}</span><span className="block truncate text-xs text-muted-foreground">{c.ultimaMensagem ?? textos.semMensagens}</span></span>{c.naoLidas > 0 && <span className="rounded-full bg-primary px-2 py-0.5 text-xs text-primary-foreground">{c.naoLidas}</span>}</button>)}
      </CardContent></Card>
      <Card className="min-h-0"><CardHeader className="p-0"><CardTitle className="sr-only">{conversaAtual?.participantes ?? textos.selecioneConversa}</CardTitle>{conversaAtual && <CabecalhoChatInterno conversa={conversaAtual} textos={textos} />}</CardHeader><CardContent className="flex min-h-0 flex-1 flex-col gap-3 p-0">
        ...
        {conversaId && <>{mensagens.isLoading ? <p className="flex flex-1 items-center justify-center text-sm text-muted-foreground">{textos.carregando}</p> : <ListaMensagensChatInterno mensagens={mensagens.data?.mensagens ?? []} usuarioAtual={usuarioAtual} textos={textos} />}<ComposerChatInterno textos={textos} enviando={enviar.isPending || enviarMidia.isPending} erro={enviar.isError || enviarMidia.isError} onEnviar={(conteudo) => enviar.mutateAsync({ id: conversaId, conteudo })} onEnviarMidia={(arquivo, legenda) => enviarMidia.mutateAsync({ id: conversaId, arquivo, legenda })} /></>}
      </CardContent></Card>
    </div>
