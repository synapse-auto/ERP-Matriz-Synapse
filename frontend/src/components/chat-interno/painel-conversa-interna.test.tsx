...
vi.mock("@/lib/config/textos-provider", () => ({
  useTextos: () => ({ atendimentos: {
    ...
    tipoGrupo: "Grupo", tipoDireta: "Conversa direta", participantesDoGrupo: "Participantes",
    sistema: { grupoCriado: "criou {nome}", participanteAdicionado: "add {alvo}", participanteRemovido: "rem {alvo}", participanteSaiu: "{alvo} saiu", nomeAlterado: "renomeou {nome}", eventoDesconhecido: "evento" },
  } }),
}));
...
describe("PainelConversaInterna", () => {
  it("renderiza autoria real, cabeçalho interno e marca leitura", async () => {
    ...
    vi.mocked(api.listarMensagensChat).mockResolvedValue({ proximoCursor: null, mensagens: [
      { id: "m1", conversaId: "c1", remetenteId: "u-atual", remetenteNome: "Ana", conteudo: "Minha mensagem https://example.org", enviadoEm: "2026-08-27T12:00:00Z" },
